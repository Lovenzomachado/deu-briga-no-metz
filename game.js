...
// Debug: muda para true para ver hitboxes e hurtboxes
const DEBUG_HITBOXES = false;

// ── Música de luta ────────────────────────────────────────────────
// Toca ao iniciar a luta, para no KO, recomeça na revanche.
const fightMusic = new Audio('music/fight_theme.mp3');
fightMusic.loop   = true;
fightMusic.volume = 0.7;

function startMusic() {
  fightMusic.currentTime = 0;
  fightMusic.play().catch(() => {}); // ignora erro de autoplay policy
}
function stopMusic() {
  fightMusic.pause();
  fightMusic.currentTime = 0;
...
// Carrega frame data de hitbox de cada personagem
player.loadMoveset();
cpu.loadMoveset();
...
function updateCPU() {
  ...
  if (dist > 280) {
    ...
    if (r < 0.30) {
      // Recua para criar distância (substituiu o bait/block)
...
function applySelection(sel) {
  ...
  // Usa charId do ROSTER (1=Henrique moveset, 2=Fezo moveset)
  player.charId = sel.player.charId ?? 1;
  cpu.charId    = sel.cpu.charId    ?? 2;
  player.loadMoveset();
  cpu.loadMoveset();
