...
// P1 — guy with cap
player.loadSprites({
  idle:    'sprites/metz/idle.png',
  stance:  'sprites/metz/stance.png',
  punch:   'sprites/metz/punch.png',
  kick:    'sprites/metz/kick.png',
  jump:    'sprites/metz/jump.png',
  block:   'sprites/metz/stance.png',
  special: [
...
function cpuAttack() {
  ...
  // No ar — usa ataques aéreos
  if (!cpu.onGround) {
    if (r < 0.5) {
      cpu.setState('air_light', 20);
      if (cpu._inRange(player, 95)) player.takeHit(12, cpu, 11);
    } else {
      cpu.setState('air_heavy', 30);
      if (cpu._inRange(player, 105)) player.takeHit(20, cpu, 16);
    }
    return;
  }
  ...
  // No chão
  if (r < 0.35) {
    cpu.setState('ground_light', 18);
    if (cpu._inRange(player, 95)) player.takeHit(15, cpu, 11);
  } else if (r < 0.6) {
    cpu.setState('ground_heavy', 30);
    if (cpu._inRange(player, 115)) player.takeHit(22, cpu, 16);
  } else if (r < 0.78) {
    cpu.setState('anti_air', 24);
    if (cpu._inRange(player, 105)) player.takeHit(16, cpu, 19, true);
  } else if (!cpu.specialUsed && r < 0.90) {
    cpu.specialUsed = true;
    cpu.setState('special', 50);
    if (cpu._inRange(player, 180)) player.takeHit(35, cpu, 19);
  } else {
    cpu.setState('down_attack', 26);
    if (cpu._inRange(player, 110)) player.takeHit(18, cpu, 12);
  }
}
...
// ── Indicador do Especial ─────────────────────────────────────────
function updateSpecialHUD() {
  const el = document.getElementById('special-indicator');
  if (!el) return;
  if (player.specialUsed) {
    el.classList.add('used');
    el.title = 'ESPECIAL USADO';
  } else {
    el.classList.remove('used');
    el.title = 'ESPECIAL DISPONÍVEL — S+D+J';
  }
}

// Callback de cura: atualiza HUD em tempo real
...
function applySelection(sel) {
  function makeSprites(folder, walkFrames, specialFrames) {
    // Tenta usar sprite dedicado; cai para punch/kick se não existir
    const s = (name, fallback) => folder + '/' + name + '.png';
    const map = {
      idle:         s('idle'),
      stance:       s('stance'),
      punch:        s('punch'),
      ...
      ground_heavy: s('ground_heavy'),
      down_attack:  s('down_attack'),
      anti_air:     s('anti_air'),
      air_light:    s('air_light'),
      air_heavy:    s('air_heavy'),
      down_air:     s('down_air'),
    };
  ...
  cpu.specialUsed     = false;
  player.healPerFrame = 0;
  cpu.healPerFrame    = 0;
  player.hitstun      = 0;
  cpu.hitstun         = 0;
  timeLeft  = TIMER_SEC;
