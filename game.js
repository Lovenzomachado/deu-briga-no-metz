...
const player = new Player({
  ...
  facing: 1, isPlayer: true, speed: 5, charId: 1,
});

const cpu = new Player({
  x: 680, y: groundY, groundY,
  width: 108, height: 136,
  facing: -1, isPlayer: false, speed: 3.2, charId: 2,
});
...
// ── Load sprites ─────────────────────────────────────────────────
// P1 — guy with cap (we have: idle, stance, punch, kick, jump)
player.loadSprites({
  ...
  block:   'sprites/p1/stance.png',   // reuse stance for block
  special: 'sprites/p1/punch.png',    // reuse punch for special
});
...
function updateCPU() {
  ...
  if (dist > 260) {
    cpu.x += (player.x > cpu.x ? 1 : -1) * cpu.speed * 14;
    cpu.setState('stance');
  } else if (dist < 80) {
    if (roll < 0.4) {
      cpu.x -= (player.x > cpu.x ? 1 : -1) * cpu.speed * 9;
      cpu.setState('stance');
    } else {
      cpuAttack();
    }
    ...
    if (roll < 0.45)      cpuAttack();
      ...
      cpu.x += (player.x > cpu.x ? 1 : -1) * cpu.speed * 7;
      cpu.setState('stance');
    }
  }
}
...
function applySelection(sel) {
  ...
  player.loadSprites(makeSprites(sel.player.folder));
  cpu.loadSprites(makeSprites(sel.cpu.folder));
...
updateHUD();
gameLoop();  // loop roda sempre (desenha o canvas mesmo na seleção)

// Mostra a tela de seleção na primeira vez
CharSelect.show(applySelection);
