...
// Em desktop 120hz: roda 2 steps por frame (física 2x, timer/HP 1x)
// Em mobile 30hz: roda 1 step a cada 2 frames (física correta)
// O timer/HP sempre 1x por frame — nunca acelera
let lastTime = 0;
const FIXED_DT = 1000 / 60; // 16.67ms por step
let accumulator = 0;

// Detecta mobile para limitar steps
...
function gameLoop(timestamp) {
  ...
  // Cap menor em mobile (16ms) para evitar múltiplos steps
  const cap = isMobileDevice ? FIXED_DT * 1.5 : 50;
  const elapsed = Math.min(timestamp - lastTime, cap);
  lastTime = timestamp;
  accumulator += elapsed;

  // Mobile: máx 1 step por frame. Desktop: máx 2 (para 120hz)
  const maxSteps = isMobileDevice ? 1 : 2;
  let steps = 0;
  ...
  // Descarta acumulador residual em mobile para não acumular dívida
  if (isMobileDevice) accumulator = 0;
  ...
  // Timer e HP: uma vez por frame (não por step — evita contar 2x em 120hz)
  if (!gameOver) {
    timerTick++;
...
// gameStep: só física, input e AI — roda N vezes por frame em alta Hz
function gameStep() {
  if (!gameOver) {
    player.update(cpu);
    updateCPU();
