...
const CharSelect = (() => {
  ...
  // ── Estado interno ─────────────────────────────────────────────
  // selectedIdx:  personagem com o cursor (hover/navegação)
  // confirmedIdx: personagem confirmado pelo jogador (-1 = ainda não)
  // onConfirmCb:  função chamada após animação VS com { player, cpu }
  // animFrame:    handle do loop de preview para cancelar ao esconder
  ...
  // Pre-load portraits ─ carrega as imagens de preview antes de mostrar a tela
  ROSTER.forEach(c => {
    const img = new Image();
    img.src = c.portrait;
    c._img = img;
  ...
  overlay.innerHTML = `
    ...
    <div id="cs-play-again" style="display:none">▶ PLAY AGAIN</div>
    <div id="cs-header">
  ...
  // ── Montar grid ───────────────────────────────────────────────
  // Cria um card HTML para cada personagem do ROSTER.
  // Click → confirma seleção. Hover → atualiza preview.
  ...
  // ── Teclado ───────────────────────────────────────────────────
  // ←/A e →/D navegam entre personagens. Enter/Espaço confirma.
  // Só processa quando o overlay está visível.
  function onKey(e) {
  ...
  // ── UI update ────────────────────────────────────────────────
  // Atualiza cards (selected highlight), painel de info e barras de stats
  // conforme o personagem em selectedIdx.
  function updateUI() {
  ...
  // ── Preview canvas ────────────────────────────────────────────
  // Desenha o sprite do personagem selecionado com bounce suave.
  // Roda a cada frame pelo previewLoop().
  ...
  // Loop independente apenas para animar o preview do personagem.
  // Para quando o overlay não está visível.
  ...
  // ── Confirmar seleção ─────────────────────────────────────────
  // Chamado ao clicar num card ou pressionar Enter.
  // CPU sempre escolhe o personagem oposto ao jogador.
  // Exibe banner VS por 1.4s antes de chamar o callback.
  function confirmSelect() {
    if (confirmedIdx !== -1) return; // já confirmou — ignora cliques duplos
  ...
  // ── Show / Hide ───────────────────────────────────────────────
  // show(cb, playAgain): exibe a tela de seleção.
  //   cb        = função chamada após seleção ({ player, cpu })
  //   playAgain = true exibe banner "PLAY AGAIN" acima da tela
  // hide(): esconde overlay e restaura o HUD do jogo.
