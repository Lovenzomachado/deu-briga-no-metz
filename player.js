...
class Player {
  ...
  _handleInput(opponent) {
    ...
    // ── Especial (↓ → + J ou S+J) — prioridade máxima ──────────────
    if (!this.specialUsed && pressJ && down && !this.locked) {
    ...
    if (this.onGround && !this.locked) {
      ...
      if (pressJ && up) {
        // Anti-air — lança o oponente levemente para cima
        this._doAttack('anti_air', 24, 16, 105, opponent, true);
        return;
      }
      if (pressJ) {
  ...
  takeHit(damage, attacker, hitstunFrames = 11, launcher = false) {
    ...
    if (!blocked) {
      ...
      // Launcher sobe o personagem levemente
      if (launcher && this.onGround) {
        this.vy       = -8;
        this.onGround = false;
      }
