...
class Player {
  ...
  _getCurrentSprite() {
    // Tenta o estado atual; cai para 'idle' se não existir
    const key    = this.sprites[this.state] ? this.state : 'idle';
    const entry  = this.sprites[key];
    if (!entry) return null;

    if (Array.isArray(entry)) {
      // Cicla pelos frames do array
      const idx = this.frame % entry.length;
      return entry[idx];
    }
    return entry;
  }
...
const ANIM_FPS = {
  idle:    6,
  walk:    10,   // walk cycle — sincronizado com os 8 frames
  stance:  8,
  punch:   14,
  kick:    14,
  jump:    10,
