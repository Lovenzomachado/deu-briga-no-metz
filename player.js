...
class Player {
  ...
  _inRange(o, r) {
    // Verifica alcance horizontal
    if (Math.abs(this.x - o.x) >= r) return false;

    // Verifica alcance vertical:
    // Se o atacante está no chão e o alvo está no ar,
    // só alcança se o alvo não estiver muito acima (altura do personagem)
    const dy = this.y - o.y; // positivo = alvo está acima do atacante
    if (this.onGround && !o.onGround) {
      // Permite acertar até ~1 altura de personagem acima
      if (dy > this.height * 0.8) return false;
    }
